        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    foreach (var item in sc.People)
                    {
                        if (item.Login == user.UserName)
                        {
                            TempData["reg"] = "<script>alert('Пользователем с таким email уже существует');</script>";
                            return RedirectToRoute(new { controller = "Account", action = "Register" });
                        }
                    }
                    sc.People.Add(new Person { Login = user.UserName, Password = model.Password });
